...
interface GridItem extends GridCoord {
  src: string;
  alt: string;
}

function buildItems(
  pool: (string | DomeGalleryImage)[],
  seg: number
): GridItem[] {
  ...
  const coords: GridCoord[] = xCols.flatMap((x, c) => {
...
export default function DomeGallery({
  ...
  dragDampening = 2,
  openedImageWidth = 'min(88vw, 400px)',
  openedImageHeight = 'min(55vh, 280px)',
    ...
    const ro = new ResizeObserver((entries) => {
      ...
      if (enlargedOverlay && frameRef.current && mainRef.current) {
        ...
        if (hasCustomSize) {
          ...
          (enlargedOverlay as HTMLElement).style.left = `${centeredLeft}px`;
          (enlargedOverlay as HTMLElement).style.top = `${centeredTop}px`;
        } else {
          (enlargedOverlay as HTMLElement).style.left = `${frameR.left - mainR.left}px`;
          (enlargedOverlay as HTMLElement).style.top = `${frameR.top - mainR.top}px`;
          (enlargedOverlay as HTMLElement).style.width = `${frameR.width}px`;
  ...
  useGesture(
    ...
    {
      target: mainRef as React.RefObject<HTMLElement>,
  ...
  useEffect(() => {
    ...
    const close = () => {
      ...
      const currentRect = (overlay as HTMLElement).getBoundingClientRect();
  ...
  const openItemFromElement = useCallback(
    (el: HTMLDivElement) => {
      ...
      const rawSrc = parent.getAttribute('data-src') || el.querySelector('img')?.getAttribute('src') || '';
